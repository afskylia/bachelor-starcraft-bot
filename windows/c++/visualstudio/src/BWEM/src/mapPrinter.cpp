...
namespace BWEM
{
	...
	namespace utils
	{
		...
		const string MapPrinter::m_fileName = "map.bmp";
		...
		const MapPrinter::Color MapPrinter::Color::chokePoints = (showAreas || showContinents)
			                                                         ? Color(255, 255, 255)
			                                                         : Color(0, 0, 0);
		...
		void MapPrinter::Line(WalkPosition A, WalkPosition B, Color col, dashed_t dashedMode)
		{
			...
			for (int i = 0; i <= N; ++i)
				Get().Point((A * i + B * (N - i)) / N, col);
		}


		void MapPrinter::Rectangle(WalkPosition TopLeft, WalkPosition BottomRight, Color col, fill_t fillMode,
		                           dashed_t dashedMode)
		{
			for (int y = TopLeft.y; y <= BottomRight.y; ++y)
				for (int x = TopLeft.x; x <= BottomRight.x; ++x)
					if ((fillMode == fill) || (y == TopLeft.y) || (y == BottomRight.y) || (x == TopLeft.x) || (x ==
						BottomRight.x))
		...
		void MapPrinter::Square(WalkPosition Center, int radius, Color col, fill_t fillMode)
		{
			for (int y = Center.y - radius; y <= Center.y + radius; ++y)
				for (int x = Center.x - radius; x <= Center.x + radius; ++x)
					if ((fillMode == fill) || (y == Center.y - radius) || (y == Center.y + radius) || (x == Center.x -
						radius) || (x == Center.x + radius))
