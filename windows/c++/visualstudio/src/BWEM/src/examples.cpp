...
namespace BWEM
{
	...
	namespace utils
	{
		void drawMap(const Map& theMap)
		{
			if (MapDrawer::showFrontier)
				for (auto f : theMap.RawFrontier())
					bw->drawBoxMap(Position(f.second), Position(f.second + 1), MapDrawer::Color::frontier,
					               static_cast<bool>("isSolid"));
			...
			for (int y = 0; y < theMap.Size().y; ++y)
				for (int x = 0; x < theMap.Size().x; ++x)
				{
					...
					{
						auto col = tile.GroundHeight() == 1
							           ? MapDrawer::Color::highGround
							           : MapDrawer::Color::veryHighGround;
						bw->drawBoxMap(Position(t), Position(t) + 6, col, static_cast<bool>("isSolid"));
						if (tile.Doodad())
							bw->drawTriangleMap(center(t) + Position(0, 5), center(t) + Position(-3, 2),
							                    center(t) + Position(+3, 2), Colors::White);
					}
				}
				...
				for (const Area& area : theMap.Areas())
					for (const ChokePoint* cp : area.ChokePoints())
						for (ChokePoint::node end : {ChokePoint::end1, ChokePoint::end2})
							bw->drawLineMap(Position(cp->Pos(ChokePoint::middle)), Position(cp->Pos(end)),
							                MapDrawer::Color::cp);

			if (MapDrawer::showMinerals)
				for (auto& m : theMap.Minerals())
				{
					bw->drawBoxMap(Position(m->TopLeft()), Position(m->TopLeft() + m->Size()),
					               MapDrawer::Color::minerals);
					if (m->Blocking())
						drawDiagonalCrossMap(Position(m->TopLeft()), Position(m->TopLeft() + m->Size()),
						                     MapDrawer::Color::minerals);
				}

			if (MapDrawer::showGeysers)
				for (auto& g : theMap.Geysers())
					bw->drawBoxMap(Position(g->TopLeft()), Position(g->TopLeft() + g->Size()),
					               MapDrawer::Color::geysers);

			if (MapDrawer::showStaticBuildings)
				for (auto& s : theMap.StaticBuildings())
				{
					bw->drawBoxMap(Position(s->TopLeft()), Position(s->TopLeft() + s->Size()),
					               MapDrawer::Color::staticBuildings);
					if (s->Blocking())
						drawDiagonalCrossMap(Position(s->TopLeft()), Position(s->TopLeft() + s->Size()),
						                     MapDrawer::Color::staticBuildings);
				}
			...
			for (const Area& area : theMap.Areas())
				for (const Base& base : area.Bases())
				{
					if (MapDrawer::showBases)
						bw->drawBoxMap(Position(base.Location()),
					...
					if (MapDrawer::showAssignedRessources)
					{
						...
						AssignedRessources.insert(AssignedRessources.end(), base.Geysers().begin(),
						                          base.Geysers().end());
		...
		{
			const WalkPosition delta(n->Pos().x < theMap.Center().x ? +1 : -1,
			                         n->Pos().y < theMap.Center().y ? +1 : -1);
			...
			for (int i = 0; i < stackSize; ++i)
			{
				...
				if (MapPrinter::showBlockingBuildings && n->Blocking())
					if (i < stackSize - 1)
					{
						...
						MapPrinter::Get().Point(WalkPosition(origin.x, (origin + size - 1).y),
						                        MapPrinter::Color::blockingNeutrals);
						MapPrinter::Get().Point(WalkPosition((origin + size - 1).x, origin.y),
						                        MapPrinter::Color::blockingNeutrals);
					}
					else
				...
				while (
					...
					(MapPrinter::showAreas &&
						any_of(area->ChokePointsByArea().begin(), area->ChokePointsByArea().end(),
						       [&map_Zone_Color, &color](
						       const pair<const Area*, const std::vector<ChokePoint>*>& neighbour)
						       {
							       ...
							       return abs(color.red - static_cast<int>(neighbouringColor.red)) +
								       abs(color.green - static_cast<int>(neighbouringColor.green)) < 150;
						       })
					));
			}
		...
		void printMap(const Map& theMap)
		{
				...
				for (int x = 0; x < theMap.WalkSize().x; ++x)
				{
					...
					else
					{
						...
						else
						{
							...
							{
								uint8_t c = static_cast<uint8_t>(255 - miniTile.Altitude() * 255 / theMap.MaxAltitude()
								);
			...
			if (MapPrinter::showData)
				for (int y = 0; y < theMap.Size().y; ++y)
					for (int x = 0; x < theMap.Size().x; ++x)
					{
						...
						uint8_t c = static_cast<uint8_t>(((data / 1) * 1) % 256);
						...
						if (groundHeight >= 1)
							for (int dy = 0; dy < 4; ++dy)
								for (int dx = 0; dx < 4; ++dx)
								{
									...
									if (theMap.GetMiniTile(p, check_t::no_check).Walkable())
										// groundHeight is usefull only for walkable miniTiles
				...
				for (const Area& area : theMap.Areas())
					for (const Base& base : area.Bases())
					{
						for (const Mineral* m : base.Minerals())
							MapPrinter::Get().Line(WalkPosition(base.Center()), WalkPosition(m->Pos()),
							                       MapPrinter::Color::bases);
						for (const Geyser* g : base.Geysers())
							MapPrinter::Get().Line(WalkPosition(base.Center()), WalkPosition(g->Pos()),
							                       MapPrinter::Color::bases);
					}
			...
			if (MapPrinter::showStartingLocations)
				for (TilePosition t : theMap.StartingLocations())
				{
					...
					MapPrinter::Get().Rectangle(origin, origin + size - 1, MapPrinter::Color::startingLocations,
					                            MapPrinter::fill);
				}
			...
			if (MapPrinter::showBases)
				for (const Area& area : theMap.Areas())
				{
					for (const Base& base : area.Bases())
					{
						...
						MapPrinter::Get().Rectangle(origin, origin + size - 1, MapPrinter::Color::bases,
						                            MapPrinter::do_not_fill, dashMode);
					}
		...
		void gridMapExample(const Map& theMap)
		{
			...
			//	Broodwar << "My " << unit->getType().getName() << " #" << unit->getID() <<
			//		" is near the center of the map." << endl;
		}
	}
