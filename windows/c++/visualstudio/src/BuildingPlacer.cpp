...
bool BuildingPlacer::canBuildHereWithSpace(BWAPI::TilePosition position, BWAPI::UnitType type, int buildDist) const
{
	...
	if (position.x > 3)
	{
		...
		for (int x = startx2; x < startx; x++)
			for (int y = starty; y < endy; y++)
			{
				auto units = BWAPI::Broodwar->getUnitsOnTile(x, y);
...
BWAPI::TilePosition BuildingPlacer::getBuildLocationNear(BWAPI::TilePosition position, BWAPI::UnitType type,
                                                         int buildDist) const
{
...
{
	for (int x = position.x; x < position.x + width && x < static_cast<int>(reserveMap.getWidth()); x++)
		for (int y = position.y; y < position.y + height && y < static_cast<int>(reserveMap.getHeight()); y++)
			reserveMap[x][y] = true;
}
...
{
	if (x < 0 || y < 0 || x >= static_cast<int>(reserveMap.getWidth()) || y >= static_cast<int>(reserveMap.getHeight()))
		return false;
